        ...
        <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <section className="bg-zinc-800/40 rounded-2xl p-6 md:p-8 border border-zinc-700/50 space-y-8">
                <div className="flex items-center gap-3 border-b border-zinc-700/50 pb-4">
                    <div className="p-2 bg-amber-500/10 rounded-lg">
                        <HelpCircle className="w-6 h-6 text-stone-200" />
                    </div>
                ...
                <div className="space-y-4">
                    ...
                    <h3 className="text-lg font-medium text-stone-200 mt-6 flex items-center gap-2">
                ...
                <div className="space-y-4 mt-8">
                    ...
                    >
                        <div className="flex gap-4">
                            ...
                            <div>
                                <strong className={`block mb-1 ${selectedOption === 'A' ? 'text-red-300' : 'text-stone-300'}`}>選項 A：</strong>
                                <span className={selectedOption === 'A' ? 'text-red-200' : 'text-stone-400'}>他太省了！他不應該只建 1 個 Bridge。3 台逆變器就應該要建 3 個不同的 Bridge，並且給它們 3 個不同的 IP 地址。</span>
                            </div>
                        </div>
                    </button>
                    ...
                    >
                        <div className="flex gap-4">
                            ...
                            <div>
                                <strong className={`block mb-1 ${selectedOption === 'B' ? 'text-green-300' : 'text-stone-300'}`}>選項 B：</strong>
                                <span className={selectedOption === 'B' ? 'text-green-200' : 'text-stone-400'}>Bridge 是無辜的！菜鳥在設定那 3 個 Device 模組時，忘記給它們設定各自獨立的<strong>「站號 (Unit ID)」</strong>，導致全部都預設成 1 號，所以 Bridge 永遠只去幫忙問 1 號機的資料。</span>
                            </div>
                        </div>
                    </button>
                    ...
                    >
                        <div className="flex gap-4">
                            ...
                            <div>
                                <strong className={`block mb-1 ${selectedOption === 'C' ? 'text-red-300' : 'text-stone-300'}`}>選項 C：</strong>
                                <span className={selectedOption === 'C' ? 'text-red-200' : 'text-stone-400'}>這是 OSGi 的錯！他一定是把 Device 對 Bridge 的依賴設定成了 STATIC，導致資料被快取卡死了。</span>
                            </div>
                        </div>
                    </button>
                </div>
                    ...
                    <div className="bg-zinc-900/50 border border-zinc-700/50 rounded-xl p-6 my-6">
                        ...
                        <ul className="space-y-3">
                            <li className="flex gap-3 items-start">
                                <span className="bg-zinc-800 px-2 flex items-center rounded text-stone-200 font-mono mt-0.5 border border-zinc-700">40100</span>
                                <span className="text-stone-300">電池當前溫度 <strong className="text-stone-200">(只能讀取)</strong></span>
                            </li>
                            <li className="flex gap-3 items-start">
                                <span className="bg-zinc-800 px-2 flex items-center rounded text-stone-200 font-mono mt-0.5 border border-zinc-700">40200</span>
                                <span className="text-stone-300">最大充電功率限制 <strong className="text-stone-200">(可以讀取，也可以由外部系統寫入修改)</strong></span>
                            </li>
                        </ul>
                    </div>
                ...
                <div className="space-y-4 mt-8">
                    ...
                    >
                        <div className="flex gap-4">
                            ...
                            <div>
                                <strong className={`block mb-1 ${selectedMappingOption === 'A' ? 'text-red-300' : 'text-stone-300'}`}>選項 A：</strong>
                                <span className={selectedMappingOption === 'A' ? 'text-red-200' : 'text-stone-400'}>把 40100 和 40200 全部塞進一個 <strong>讀取任務 (FC3)</strong> 裡就好，反正 OpenEMS 這麼聰明，我要修改功率時它自己會結合在一起寫入。</span>
                            </div>
                        </div>
                    </button>
                    ...
                    >
                        <div className="flex gap-4">
                            ...
                            <div>
                                <strong className={`block mb-1 ${selectedMappingOption === 'B' ? 'text-green-300' : 'text-stone-300'}`}>選項 B：</strong>
                                <span className={selectedMappingOption === 'B' ? 'text-green-200' : 'text-stone-400'}>40100 放在讀取任務 (FC3)。至於 40200，因為它需要讀取當前設定，又需要被寫入，所以我必須把它<strong>同時放進 讀取任務 (FC3) 和 寫入任務 (FC16) 裡。</strong></span>
                            </div>
                        </div>
                    </button>
                </div>
                    ...
                    <div className={`mt-6 p-5 rounded-xl border duration-300 animate-in fade-in slide-in-from-top-2 ${selectedMappingOption === 'B'
                        ...
                        <div className="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <div className="leading-relaxed opacity-90 text-stone-200">
                                    ...
                                    {selectedMappingOption === 'B' && (
                                        <div className="space-y-5 mt-3">
                                            ...
                                            <div className="bg-zinc-900/50 rounded-xl p-5 border border-green-500/20 text-stone-100 shadow-inner">
                                            ...
                                            <div className="bg-zinc-950/80 rounded-xl overflow-hidden border border-zinc-700/50 shadow-md">
                                                <div className="bg-zinc-800/80 px-4 py-3 border-b border-zinc-700/50">
                                                    <span className="font-mono text-stone-200 text-sm font-medium flex items-center gap-2">
                                            ...
                                            <div className="bg-amber-900/10 rounded-xl p-5 border border-amber-500/20 flex gap-4 items-start shadow-sm mt-2">
                                                ...
                                                <div>
                                                    <h5 className="text-stone-200 font-bold mb-2">呼應第一階段的記憶點：還記得 Doc.of() 嗎？</h5>
                                                    ...
                                                    <div className="bg-zinc-950 p-3 rounded-md border border-zinc-800">
                                                        <code className="text-stone-400 text-sm font-mono break-all">
            ...
            <section className="bg-gradient-to-br from-indigo-900/40 to-blue-900/20 rounded-2xl p-6 md:p-8 border border-indigo-500/30 space-y-6 mb-8 shadow-lg shadow-indigo-900/20">
                ...
                <div className="grid lg:grid-cols-3 gap-6 mt-6">
                    ...
                    <div className="bg-zinc-900/70 rounded-xl p-6 border border-zinc-700/50 flex flex-col h-full hover:border-emerald-500/30 transition-colors">
                        ...
                        <ul className="space-y-3 text-stone-300/80 text-sm md:text-base flex-1">
                            <li><strong className="text-stone-100">OT 端 (屋頂)：</strong> 設備使用 Modbus RTU (RS485) 串接，必須設定獨立的 <span className="text-stone-200">站號 (Unit ID)</span>。</li>
                            <li><strong className="text-stone-100">Gateway (變形金剛)：</strong> 將 RS485 電訊號轉為 TCP 網路封包。</li>
                            <li><strong className="text-stone-100">IT 端 (機房)：</strong> 透過一樓網路線 (Cat.6) 以 <span className="text-stone-200">IP 地址</span> 尋找 Gateway。</li>
                        </ul>
                    </div>
                    ...
                    <div className="bg-zinc-900/70 rounded-xl p-6 border border-zinc-700/50 flex flex-col h-full hover:border-blue-500/30 transition-colors">
                        ...
                        <ul className="space-y-3 text-stone-300/80 text-sm md:text-base flex-1">
                            <li><strong className="text-stone-100">Bridge (翻譯官)：</strong> 負責實體連線，填寫 Gateway IP。</li>
                            <li><strong className="text-stone-100">Device (外籍員工)：</strong> 設備邏輯模組，填寫 Unit ID。</li>
                    ...
                    <div className="bg-zinc-900/70 rounded-xl p-6 border border-zinc-700/50 flex flex-col h-full hover:border-purple-500/30 transition-colors">
                        ...
                        <ul className="space-y-3 text-stone-300/80 text-sm md:text-base flex-1">
                            <li><strong className="text-stone-100">FC3 (讀取)：</strong> 每秒問現況 (如當前溫度)。</li>
                            <li><strong className="text-stone-100">FC16 (寫入)：</strong> 改變狀態時觸發 (如命令放電)。</li>
                            <li><strong className="text-stone-100">m() 魔法：</strong> 把硬體地址綁到這兩類的 Channel 上。</li>
                            <li><strong className="text-stone-300">雙向綁定：</strong> 可修改的屬性須同時放入 FC3 與 FC16，這就是 <code>READ_WRITE</code>。</li>
                        </ul>
                    </div>
                </div>
            </section>
            ...
            <section className="bg-zinc-800/80 border border-zinc-700/80 rounded-2xl p-6 md:p-8 relative overflow-hidden">
                ...
                <div className="prose prose-invert max-w-none mb-8">
                    <p className="text-stone-300 leading-relaxed text-lg">
                        這題綜合測驗，將會同時考驗你 <strong className="text-stone-200">第一階段：Channel 身分證</strong> 與 <strong className="text-stone-200">第三階段：Modbus 任務映射</strong> 的融會貫通能力。準備接招！
                    </p>

                    <div className="bg-zinc-900 rounded-xl p-5 border border-zinc-700 mt-6">
                        <h4 className="text-stone-200 font-semibold mb-3 flex items-center gap-2">
                            【案發現場】
                        </h4>
                        ...
                        <ul className="text-stone-300 space-y-2 mt-3 list-disc list-inside">
                            <li><strong className="text-stone-200">地址 40010：</strong> 當前總耗電量 (僅供讀取)</li>
                            <li><strong className="text-stone-200">地址 40050：</strong> 警報解除開關 (寫入 1 即可解除電表上的蜂鳴器警報；平時讀取會顯示目前的警報狀態)</li>
                        </ul>
                    </div>
                ...
                <div className="space-y-4">
                    ...
                    >
                        <div className="flex gap-4">
                            ...
                            <div>
                                <strong className={`block mb-1 ${selectedBossOption === 'A' ? 'text-red-300' : 'text-stone-100'}`}>選項 A：Modbus 任務衝突了！</strong>
                                <span className={selectedBossOption === 'A' ? 'text-red-200' : 'text-stone-400'}>因為 `ALARM_STATUS` 同時被放在 FC3 (讀取) 和 FC16 (寫入) 的任務裡。這兩個任務會互相打架，導致寫入失敗。應該要把 FC3 裡面的 `ALARM_STATUS` 刪掉。</span>
                            </div>
                        </div>
                    </button>
                    ...
                    >
                        <div className="flex gap-4">
                            ...
                            <div>
                                <strong className={`block mb-1 ${selectedBossOption === 'B' ? 'text-green-300' : 'text-stone-100'}`}>選項 B：Channel 的身分類型錯了！</strong>
                                <span className={selectedBossOption === 'B' ? 'text-green-200' : 'text-stone-400'}>Modbus Protocol Mapping 寫得完全正確，錯誤出在 `Meter` 這個介面的 Channel 定義上。當初在寫 `ALARM_STATUS` 的身分證時，一定是不小心用了 `Doc.of(OpenemsType.BOOLEAN)`，而沒有加上 `.accessMode(AccessMode.READ_WRITE)`，導致它被判定成只能讀取的唯讀 Channel。</span>
                            </div>
                        </div>
                    </button>
                    ...
                    >
                        <div className="flex gap-4">
                            ...
                            <div>
                                <strong className={`block mb-1 ${selectedBossOption === 'C' ? 'text-red-300' : 'text-stone-100'}`}>選項 C：地址映射跨距太大！</strong>
                                <span className={selectedBossOption === 'C' ? 'text-red-200' : 'text-stone-400'}>在 FC3 讀取任務中，你把地址 40010 和 40050 包在同一個 Task 裡面。因為中間跳躍了 40 個地址，這會導致硬體無法解讀，進而造成後續的寫入任務也跟著當機。</span>
                            </div>
                        </div>
                    </button>
                </div>
                                ...
                                <div className="space-y-4 opacity-90 text-stone-200">
                                    ...
                                    {selectedBossOption === 'B' && (
                                        <>
                                            ...
                                            <p className="mt-2 text-stone-100">雖然你在 <strong className="text-stone-200">第三階 (Modbus)</strong> 把它綁到了 FC16 寫入任務，但因為你在 <strong className="text-stone-200">第一階 (Channel 介面宣告)</strong> 時，忘記幫它申請 <code>READ_WRITE</code> 的權限，導致 OpenEMS 核心大腦把它當成「唯讀」的。<br /><br />
